...
def evaluate(data_loader, model, device, args):
    ...
    print('eval_acc', round(overall_acc,4))
    print('eval_auc', round(overall_auc,4))

    for ii in range(len(eval_es_acc)):
        print(f'eval_es_acc_attr{ii}', round(eval_es_acc[ii],4))
    for ii in range(len(eval_es_auc)):
        print(f'eval_es_auc_attr{ii}', round(eval_es_auc[ii],4))
    for ii in range(len(eval_aucs_by_attrs)):
        for iii in range(len(eval_aucs_by_attrs[ii])):
            print(f'eval_auc_attr{ii}_group{iii}', round(eval_aucs_by_attrs[ii][iii],4))

    for ii in range(len(between_group_disparity)):
        print(f'eval_auc_attr{ii}_std_group_disparity', round(between_group_disparity[ii][0],4))
        print(f'eval_auc_attr{ii}_max_group_disparity', round(between_group_disparity[ii][1],4))

    for ii in range(len(eval_dpds)):
        print(f'eval_dpd_attr{ii}', round(eval_dpds[ii],4))
    for ii in range(len(eval_eods)):
        print(f'eval_eod_attr{ii}', round(eval_eods[ii],4))
